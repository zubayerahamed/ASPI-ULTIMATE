...
(function($) {
  ...
  $.fn.smkValidate = function() {
    ...
    if (languaje === undefined) {
      ...
      languaje =  {
        ...
        textConStart     : 'From date can\'t occur after to date'
      };
    }
    ...
    inputs.each(function(k,v) {
      ...
      var smkConStart = $(v).attr('con-start');
      ...
      // Date range validation
      if (type === 'datepicker' && smkConStart === 'true'){
    	  console.log("%cValidate date range", "color:green");
    	  var startDate = input;
		  var endDate = $('input[con-start="false"]');
		  if(startDate != '' && endDate != ''){
			  var stat = moment(endDate.val(), "DD-MMM-YYYY") >= moment(startDate.val(), "DD-MMM-YYYY");
			  if(stat == false){
				  result =  $.smkAddError(input, languaje.textConStart);
			  }
		  }
      }
      ...
      if (value !== '') {
        ...
        if (type === 'numberx' || smkType === 'numberx') {
  ...
  */
  $.smkAddError = function (obj, message)
  {
    ...
    if(type === undefined) type = $(obj).attr('data-type');
    ...
    if(type == 'select'){
      ...
      formGroup.find('.form-control').parent().append(msg);
    }else if(type == 'checkbox' || type == 'radio'){
    	formGroup.addClass('has-feedback has-error');
    	if(formGroup.hasClass('input2')){
    		msg = $(msg).addClass('col-sm-offset-4 input2-error');
    	...
    	formGroup.addClass('has-feedback has-error');
    	if(formGroup.hasClass('timerange')){
    		msg = $(msg).addClass('col-sm-offset-4');
    		formGroup.append(msg);
    	} else {
    		formGroup.find('.form-control').parent().append(msg);
    	}
    } else {
    	// check input is datepicker
