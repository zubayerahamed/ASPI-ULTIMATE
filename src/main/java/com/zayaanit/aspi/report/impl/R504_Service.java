...
@Service(value = "R504_Service")
public class R504_Service extends AbstractReportService {
	...
	public List<FormFieldBuilder> getReportFields() {
		...
		List<Xcodes> issueTypes = xcodesRepo.findAllByXtypeAndZactiveAndZid("IM Issue Type", Boolean.TRUE, sessionManager.getBusinessId());
		issueTypes.stream().forEach(f -> {
			status.add(new DropdownOption(f.getXcode(), f.getXcode()));
		});
		...
		fieldsList.add(FormFieldBuilder.generateDropdownField(7, "Item Group", itemGroups, "", false));
		...
		fieldsList.add(FormFieldBuilder.generateDropdownField(8, "Item Category", itemCategory, "", false));

		fieldsList.add(FormFieldBuilder.generateAdvancedSearchField(9, "Item", "/search/table/LMD12/1?hint=", "", false));
